...
export async function buildAgentsMd(opts: ContextPipelineOptions): Promise<{
  ...
  // NOTE: keep injection-scanner-safe wording. Hermes 0.21's context-file
  // threat scan blocks any HTML comment containing words like "override"
  // (tools/threat_patterns.py html_comment_injection) — and
  // "AGENTS.override.md" contains that word, so the old
  // "<!-- Generated by AIWG. Edit AGENTS.override.md ... -->" marker got
  // whole-file BLOCKED on every Hermes turn. The functional signature the
  // pipeline needs is `<!-- aiwg-managed -->` alone; the override pointer
  // lives in plain prose below, outside any comment.
  parts.push('Operator additions: edit `AGENTS.override.md` (loaded after this file by Hermes).');
  parts.push('');
 ...
 * Returns `false` if the file exists and does not carry the canonical signature
 * comment in its first 4 lines (operator-claimed or ambiguous).
 *
 * Pre-ADR-1 files without the signature are treated as operator-owned: the
 * legacy-marker heuristic this function once used (`<!-- AIWG ...` anywhere in
 * the first 50 lines) also matched operator documents that merely reference
 * AIWG — e.g. an operator AGENTS.md carrying an appended AIWG managed block or
 * a mention of `aiwg use sdlc` — and the caller then replaced the whole file,
 * silently destroying the operator prefix. Ambiguity now fails closed: such
 * files take the additive-hook path (#1597), and converting them fully
