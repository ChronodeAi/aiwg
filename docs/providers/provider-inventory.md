...
| `dsh` | DeepSeek Harness (fork preset integration) | Experimental | Mixed project/user |

`agy` is the only alias for `antigravity` and is also Google's CLI executable.
`dsh` (alias `deepseek`) selects the fork preset integration, preserving its
OpenRouter-first model policy and Cordis configuration. `deepseek-harness`
selects the separate upstream runtime and has no aliases. Both use the `dsh`
executable; their AIWG provider selectors are deliberately distinct.
