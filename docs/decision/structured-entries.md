...
The runtime registers v1alpha1 and v1alpha2 schemas for all five artifact kinds. v1alpha1 schemas remain strict with `additionalProperties: false`; a changed field cannot be smuggled into an old document. The current v1alpha2 ruleset and binding schemas are deliberately shape-compatible until D07–D10 add their fields. The contract test validates both versions and proves an undeclared acceptance field is rejected. D07–D10 must extend only their v1alpha2 schemas/types and add concrete fixtures for their fields before those semantics can be authored. This matrix is a writer gate, not an implementation claim for unfinished fields.
