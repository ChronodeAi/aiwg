...
export async function deploySkills(targetDir, srcRoot, opts) {
  ...
  await new Promise((resolve, reject) => {
    ...
    // Direct-addon deployers (e.g. PMOS) own their placement and reject
    // unknown flags, so the listing-budget flags go to the canonical helper only.
    if (!directAddon) {
    ...
    // A copy-all the deployer forced on the caller's behalf (project-local
    // bundles) still honors the startup listing cap (#2561).
    if (opts.listingBudget === true && !directAddon) args.push('--listing-budget');
