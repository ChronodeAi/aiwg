...
// Hermes 0.21 subagent context note. Byte-for-byte shared with
// src/smiths/context-pipeline/hermes-contract.ts (HERMES_SUBAGENT_NOTE) —
// the sync test test/unit/smiths/context-pipeline-hermes.test.ts fails if
// either copy drifts. Kept verbatim (not a paraphrase) so the pipeline
// (regenerate path) and the direct-deploy generator emit identical text.
const HERMES_SUBAGENT_NOTE = `## Hermes Subagents (0.21+)
...
<!-- aiwg-managed -->
<!-- Managed by AIWG; regenerated on deploy. -->
...
Use \`delegate_task\` for AIWG workflows — see the Hermes Subagents section below.
